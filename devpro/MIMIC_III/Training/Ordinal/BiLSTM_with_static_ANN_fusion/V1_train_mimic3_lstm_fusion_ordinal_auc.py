...
OUT_DIR = os.path.join(_PROJECT_ROOT, "results_LOS_dynamic", "mimic3_lstm_fusion_ordinal_auc_models_v1")
...
class FusionLSTMOrdinal(nn.Module):
    def __init__(self, input_dyn_dim, input_static_dim, hidden=384, layers=2, dropout=0.3, static_hidden=128):
        ...
        self.head = build_standard_head(fusion_in_dim, n_classes=2)  # 2 ordinal logits: P(LOS>3), P(LOS>7)
...
def main():
    ...
    opt = build_standard_optimizer(model, lr=args.lr)
    scheduler = build_standard_scheduler(opt)
