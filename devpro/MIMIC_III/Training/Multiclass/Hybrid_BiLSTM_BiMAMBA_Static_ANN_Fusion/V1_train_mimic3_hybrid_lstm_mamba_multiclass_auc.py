...
MIMIC-III Hybrid BiLSTM + BiMamba + Static ANN Fusion plain 3-class
(multiclass) LOS prediction. Plain softmax/CrossEntropy over 3 classes --
NOT the cumulative-logit ordinal formulation under Training/Ordinal/.

Run:
    python V1_train_mimic3_hybrid_lstm_mamba_multiclass_auc.py --workers 0 --batch 128
"""
...
OUT_DIR = os.path.join(_PROJECT_ROOT, "results_timeseries", "mimic3_hybrid_lstm_mamba_multiclass_auc_models_v1")
...
class DynStaticMulticlassDataset(Dataset):
    def __init__(self, X_dyn, X_static, y_3c, copy_mmap=True):
        self.Xd = X_dyn
        self.Xs = X_static
...
class BiMamba(nn.Module):
    def __init__(self, f_in, d_model=128, layers=2, resid_scale=0.5):
        super().__init__()
...
class HybridDynEncoder(nn.Module):
    def __init__(self, f_dyn, lstm_hidden=256, mamba_dim=128):
        super().__init__()
        self.lstm = nn.LSTM(
            input_size=f_dyn,
...
class HybridMulticlassModel(nn.Module):
    def __init__(self, f_dyn, f_static):
        super().__init__()
        self.dyn = HybridDynEncoder(f_dyn=f_dyn, lstm_hidden=256, mamba_dim=128)
        ...
        self.head = build_standard_head(4 * 256 + 4 * 128 + 128, n_classes=N_CLASSES)
...
def main():
    ...
    print("USING MIMIC-III HYBRID LSTM + MAMBA MULTICLASS (3-CLASS SOFTMAX) WITH AUROC/AUPRC")
    ...
    model = HybridMulticlassModel(f_dyn=Xd_tr.shape[2], f_static=Xs_tr.shape[1]).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        va_loss, va_probs, va_true = run_epoch(model, va_loader, device, criterion, optimizer=None, grad_clip=args.grad_clip, amp=args.amp)
    ...
    te_loss, te_probs, te_true = run_epoch(model, te_loader, device, criterion, optimizer=None, grad_clip=args.grad_clip, amp=args.amp)
    ...
    stamp = f"mimic3_hybrid_lstm_mamba_multiclass_ep{best_epoch}_Fdyn{Xd_tr.shape[2]}_Fstat{Xs_tr.shape[1]}"
    model_path = os.path.join(OUT_DIR, f"hybrid_multiclass_{stamp}.pt")
    ...
    torch.save(
        {
            ...
            "model": "Hybrid BiLSTM + BiMamba + static ANN fusion multiclass",
            **te_metrics,
        },
        model_path,
    )
