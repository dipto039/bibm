#!/usr/bin/env python3
"""
V1_train_mimic3_hybrid_lstm_mamba_binary_auc.py

MIMIC-III Hybrid BiLSTM + BiMamba + Static ANN Fusion Binary.

Run:
    python V1_train_mimic3_hybrid_lstm_mamba_binary_auc.py --workers 0 --batch 128
"""
...
OUT_DIR = os.path.join(_PROJECT_ROOT, "results_LOS_dynamic", "mimic3_hybrid_lstm_mamba_binary_auc_models_v1")
...
class DynStaticBinaryDataset(Dataset):
    def __init__(self, X_dyn, X_static, y_bin, copy_mmap=True):
        self.Xd = X_dyn
        self.Xs = X_static
...
class HybridBinaryModel(nn.Module):
    def __init__(self, f_dyn, f_static):
        ...
        self.head = build_standard_head(4 * 256 + 4 * 128 + 128, n_classes=2)
...
def main():
    ...
    print("USING MIMIC-III HYBRID LSTM + MAMBA BINARY WITH AUROC/AUPRC")
    ...
    model = HybridBinaryModel(f_dyn=Xd_tr.shape[2], f_static=Xs_tr.shape[1]).to(device)
    ...
    stamp = f"mimic3_hybrid_lstm_mamba_binary_ep{best_epoch}_Fdyn{Xd_tr.shape[2]}_Fstat{Xs_tr.shape[1]}"
    ...
    print(
        ...
        f"F1 weighted: {te_f1_weighted:.4f} | AUROC: {te_auroc:.4f} | "
        f"AUPRC: {te_auprc:.4f} | Test loss: {te_loss:.4f} | "
