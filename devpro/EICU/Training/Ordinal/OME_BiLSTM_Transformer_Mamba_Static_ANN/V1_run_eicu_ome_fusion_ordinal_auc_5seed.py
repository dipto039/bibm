#!/usr/bin/env python3
"""
V1_run_eicu_ome_fusion_ordinal_auc_5seed.py

5-seed runner for eICU OME-Fusion Ordinal with AUROC/AUPRC.

Reuses:
    V1_train_eicu_ome_fusion_ordinal_auc.py

Creates seed-specific temp copies and changes:
    seed_everything(42) -> seed_everything(<seed>)
    OUT_DIR -> /lustre/home/rahas2/mimic_projects/outputs/eicu_ome_fusion_ordinal_auc_5seed_models_v1/seed_<seed>

Run:
    python V1_run_eicu_ome_fusion_ordinal_auc_5seed.py --workers 0 --batch 128
"""
...
RUN_ROOT = f"{BASE_OUT}/eicu_ome_fusion_ordinal_auc_5seed_models_v1"
SOURCE_SCRIPT = "V1_train_eicu_ome_fusion_ordinal_auc.py"
...
def patch_script(src_text: str, seed: int, seed_out_dir: str) -> str:
    ...
    text = text.replace(
        'OUT_DIR = f"{BASE}/eicu_ome_fusion_ordinal_auc_models_v1"',
    ...
    text = text.replace(
        'print("USING eICU OME-FUSION ORDINAL 3-CLASS WITH AUROC/AUPRC")',
        f'print("USING eICU OME-FUSION ORDINAL 3-CLASS WITH AUROC/AUPRC | SEED {seed}")'
    )
...
def summarize(all_metrics, summary_dir):
    ...
    json_path = os.path.join(summary_dir, "summary_eicu_ome_fusion_ordinal_auc_5seed.json")
    txt_path = os.path.join(summary_dir, "summary_eicu_ome_fusion_ordinal_auc_5seed.txt")

    with open(json_path, "w") as f:
        json.dump(summary, f, indent=2)

    lines = [
        "eICU OME-Fusion Ordinal | 5-Seed Summary",
        "BiLSTM + Transformer + Mamba + Static ANN",
        "",
        "Mean ± SD:",
    ]
    ...
    for m in all_metrics:
        line = (
            ...
            f"auroc_gt3={m['test_auroc_gt3']:.4f} | "
            f"auroc_gt7={m['test_auroc_gt7']:.4f} | "
            f"auprc_gt3={m['test_auprc_gt3']:.4f} | "
            f"auprc_gt7={m['test_auprc_gt7']:.4f}"
        )
        lines.append(line)
...
def main():
    ...
    for seed in args.seeds:
        ...
        tmp_script = script_dir / f"_tmp_eicu_ome_fusion_ordinal_auc_seed{seed}.py"
        ...
        cmd = [
            ...
            "--mamba_layers", str(args.mamba_layers),
