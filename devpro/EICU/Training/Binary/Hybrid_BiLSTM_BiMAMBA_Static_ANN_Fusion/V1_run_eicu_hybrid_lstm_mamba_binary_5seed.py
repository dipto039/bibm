#!/usr/bin/env python3
"""
V1_run_eicu_hybrid_lstm_mamba_binary_5seed.py

5-seed runner for Hybrid BiLSTM + BiMamba + Static ANN Fusion Binary.

Reuses:
    V1_train_eicu_hybrid_lstm_mamba_binary_auc.py

Creates seed-specific output folders:
    /lustre/home/rahas2/mimic_projects/outputs/eicu_hybrid_lstm_mamba_binary_auc_5seed_models_v1/seed_<seed>

Reports:
    Acc, Macro F1, Weighted F1, AUROC, AUPRC

Run:
    python V1_run_eicu_hybrid_lstm_mamba_binary_5seed.py --workers 0 --batch 128
"""
...
RUN_ROOT = f"{BASE_OUT}/eicu_hybrid_lstm_mamba_binary_auc_5seed_models_v1"
SOURCE_SCRIPT = "V1_train_eicu_hybrid_lstm_mamba_binary_auc.py"
...
def patch_script(src_text: str, seed: int, seed_out_dir: str) -> str:
    ...
    text = text.replace(
        ...
        f'print("USING eICU HYBRID LSTM + MAMBA BINARY WITH AUROC/AUPRC | SEED {seed}")'
    )
...
def summarize(all_metrics, summary_dir):
    ...
    json_path = os.path.join(summary_dir, "summary_hybrid_lstm_mamba_binary_5seed.json")
    txt_path = os.path.join(summary_dir, "summary_hybrid_lstm_mamba_binary_5seed.txt")

    with open(json_path, "w") as f:
        json.dump(summary, f, indent=2)

    lines = [
        "Hybrid BiLSTM + BiMamba + Static ANN Fusion Binary | 5-Seed Summary",
        "",
    ]
...
def main():
    ...
    for seed in args.seeds:
        ...
        tmp_script = script_dir / f"_tmp_hybrid_lstm_mamba_binary_seed{seed}.py"
