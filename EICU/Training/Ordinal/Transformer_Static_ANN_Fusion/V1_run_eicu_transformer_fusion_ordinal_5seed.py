#!/usr/bin/env python3
"""
V1_run_eicu_transformer_fusion_ordinal_5seed.py
-----------------------------------------------

5-seed runner for Transformer + Static ANN Fusion Ordinal.

This script reuses:
    V1_train_eicu_transformer_fusion_ordinal_auc.py

It creates temporary seed-specific copies, changes:
    seed_everything(42) -> seed_everything(<seed>)
    OUT_DIR -> /lustre/home/rahas2/mimic_projects/outputs/eicu_transformer_fusion_ordinal_5seed_models_v1/seed_<seed>

Then it runs each seed and creates:
    summary_transformer_ordinal_5seed.json
    summary_transformer_ordinal_5seed.txt

Run:
    python V1_run_eicu_transformer_fusion_ordinal_5seed.py --workers 0 --batch 128

Optional:
    python V1_run_eicu_transformer_fusion_ordinal_5seed.py --seeds 42 43 44 45 46 --workers 0 --batch 128
"""
...
RUN_ROOT = f"{BASE_OUT}/eicu_transformer_fusion_ordinal_5seed_models_v1"
SOURCE_SCRIPT = "V1_train_eicu_transformer_fusion_ordinal_auc.py"
...
def patch_script(src_text: str, seed: int, seed_out_dir: str) -> str:
    ...
    text = text.replace(
        ...
        f'print("USING eICU TRANSFORMER + ANN FUSION ORDINAL 3-CLASS WITH AUROC/AUPRC | SEED {seed}")'
    )
...
def summarize(all_metrics, summary_dir):
    ...
    # Weighted F1 is inside sklearn classification_report.
    weighted_vals = [
    ...
    json_path = os.path.join(summary_dir, "summary_transformer_ordinal_5seed.json")
    txt_path = os.path.join(summary_dir, "summary_transformer_ordinal_5seed.txt")

    with open(json_path, "w") as f:
        json.dump(summary, f, indent=2)

    lines = []
    lines.append("Transformer + Static ANN Fusion Ordinal | 5-Seed Summary")
    lines.append("")
    ...
    for m in all_metrics:
        ...
        lines.append(
            ...
            f"auroc_macro={m['test_auroc_macro']:.4f} | "
            f"auprc_macro={m['test_auprc_macro']:.4f}"
        )
...
def main():
    ...
    for seed in args.seeds:
        ...
        tmp_script = script_dir / f"_tmp_transformer_ordinal_auc_seed{seed}.py"
