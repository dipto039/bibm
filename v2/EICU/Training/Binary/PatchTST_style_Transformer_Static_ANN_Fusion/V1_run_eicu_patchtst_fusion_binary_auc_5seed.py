#!/usr/bin/env python3
"""
V1_run_eicu_patchtst_fusion_binary_auc_5seed.py

5-seed runner for eICU PatchTST-style Transformer + Static ANN Fusion Binary with AUROC/AUPRC.
...
Creates seed-specific temp copies and changes:
    ...
    OUT_DIR -> /lustre/home/rahas2/mimic_projects/outputs/eicu_patchtst_fusion_binary_auc_5seed_models_v1/seed_<seed>

Run:
    python V1_run_eicu_patchtst_fusion_binary_auc_5seed.py --workers 0 --batch 128
"""
...
RUN_ROOT = f"{BASE_OUT}/eicu_patchtst_fusion_binary_auc_5seed_models_v1"
SOURCE_SCRIPT = "V1_train_eicu_patchtst_fusion_binary_auc.py"
...
def patch_script(src_text: str, seed: int, seed_out_dir: str) -> str:
    ...
    text = text.replace(
        ...
        f'print("USING eICU PATCHTST-STYLE TRANSFORMER + ANN FUSION BINARY WITH AUROC/AUPRC | SEED {seed}")'
    )
...
def summarize(all_metrics, summary_dir):
    ...
    json_path = os.path.join(summary_dir, "summary_eicu_patchtst_fusion_binary_auc_5seed.json")
    txt_path = os.path.join(summary_dir, "summary_eicu_patchtst_fusion_binary_auc_5seed.txt")

    with open(json_path, "w") as f:
        json.dump(summary, f, indent=2)

    lines = [
        "eICU PatchTST-style Transformer + Static ANN Fusion Binary | 5-Seed Summary",
        "",
        "Mean ± SD:",
    ]
...
def main():
    ...
    for seed in args.seeds:
        ...
        tmp_script = script_dir / f"_tmp_eicu_patchtst_fusion_binary_auc_seed{seed}.py"
